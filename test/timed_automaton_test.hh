...
public:
  ...
  std::unordered_map<std::shared_ptr<TAState>, std::shared_ptr<TAState>> old2new;
};
...
BOOST_FIXTURE_TEST_CASE( deepCopyIdenticalTest, DeepCopyFixture )
{
  ...
  for (auto oldState: TA.states) {
    auto newState = old2new[oldState];
...
BOOST_FIXTURE_TEST_CASE( deepCopyImmutable, DeepCopyFixture )
{
  ...
  BOOST_CHECK_EQUAL(old2new[TA.states[0]]->next['a'][0].resetVars.size(), 1);
}
