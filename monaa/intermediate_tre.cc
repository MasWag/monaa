...
DNFTRE::DNFTRE(const std::shared_ptr<const TRE> tre) {
  ...
  case TRE::op::concat: {
    ...
    std::shared_ptr<DNFTRE> subfmlLeft =
        std::make_shared<DNFTRE>(tre->regExprPair.first);
    std::shared_ptr<DNFTRE> subfmlRight =
        std::make_shared<DNFTRE>(tre->regExprPair.second);
  ...
  case TRE::op::conjunction: {
    ...
    for (const auto &conjunctionsLeft : subfmlLeft->list) {
      for (const auto &conjunctionsRight : subfmlRight->list) {
        ...
        conjunctions.insert(conjunctions.end(), conjunctionsRight.begin(),
                            conjunctionsRight.end());
...
void AtomicTRE::toNormalForm() {
  switch (tag) {
  case op::singleton: {
    decision = std::make_shared<SyntacticDecision>(
        SyntacticDecision::Decision::Constant,
        std::vector<Alphabet>{singleton->c});
    break;
  }
  case op::epsilon: {
    decision = std::make_shared<SyntacticDecision>(
        SyntacticDecision::Decision::Constant, std::vector<Alphabet>{0});
    break;
  }
  case op::concat: {
    decision = std::make_shared<SyntacticDecision>(
    ...
    if (tag == op::concat) {
      auto it =
          find_if(list.begin(), list.end(), [](std::shared_ptr<AtomicTRE> tre) {
            return tre->tag == op::plus && tre->expr->decision->isMixed();
          });
      if (it != list.end()) {
        // when the subformula is splitted (not plus anymore)
        // propergate within
        std::list<std::shared_ptr<AtomicTRE>> prevList = {list.begin(),
                                                          std::prev(it)};
        std::list<std::shared_ptr<AtomicTRE>> afterList = {std::next(it),
                                                           list.end()};
  ...
  case op::plus: {
    expr->toNormalForm();
    decision = std::make_shared<SyntacticDecision>(
        SyntacticDecision::Decision::Mixed, expr->decision->chars);
    if (expr->decision->tag == SyntacticDecision::Decision::Constant &&
        (decision->chars.size() == 1 ||
         (decision->chars.size() == 2 && decision->chars[0] == 0))) {
    ...
    if (decision->isMixed()) {
      ...
      expr->list.sort();
      std::vector<std::list<std::shared_ptr<AtomicTRE>>> vec(expr->list.begin(),
                                                             expr->list.end());
      for (uint32_t i = 1; i < subsetSize; i++) {
        ...
        do {
          ...
          for (auto &conjunctions : subSetVec) {
            atomic = std::make_shared<AtomicTRE>(
                atomic, std::make_shared<AtomicTRE>(
                            std::make_shared<DNFTRE>(conjunctions)));
          }
  ...
  case op::within: {
    ...
    decision = std::make_shared<SyntacticDecision>(
        within.first->decision->tag, within.first->decision->chars);
    ...
    case op::singleton: {
      ...
      tag = op::singleton;
      new (&singleton)
          std::shared_ptr<SingletonTRE>(std::move(origExpr->singleton));
      break;
    }
    case op::epsilon: {
      within.~pair<std::shared_ptr<AtomicTRE>, std::shared_ptr<Interval>>();
...
void DNFTRE::toNormalForm() {
  ...
  for (auto it = list.begin(); it != list.end();) {
    auto itc = std::find_if(
        it->begin(), it->end(), [](std::shared_ptr<AtomicTRE> expr) {
    ...
    if (itc != it->end()) {
      ...
      std::list<std::shared_ptr<AtomicTRE>> others = {it->begin(),
                                                      std::prev(itc)};
  ...
  for (auto &conjunctions : list) {
    ...
    decisions.push_back(
        std::make_shared<SyntacticDecision>(*((*it)->decision)));
    for (it++; it != conjunctions.end();) {
      decisions.back()->tag =
          (decisions.back()->tag == SyntacticDecision::Decision::Constant ||
           (*it)->decision->tag == SyntacticDecision::Decision::Constant)
              ? SyntacticDecision::Decision::Constant
              : SyntacticDecision::Decision::Mixed;
      std::vector<Alphabet> chars(decisions.back()->chars.size());
      chars.resize(std::set_intersection(decisions.back()->chars.begin(),
                                         decisions.back()->chars.end(),
                                         (*it)->decision->chars.begin(),
                                         (*it)->decision->chars.end(),
                                         chars.begin()) -
                   chars.begin());
  ...
  for (it++; it != decisions.end(); it++) {
    decision->tag = (decision->tag == SyntacticDecision::Decision::Constant &&
                     (*it)->tag == SyntacticDecision::Decision::Constant)
                        ? SyntacticDecision::Decision::Constant
                        : SyntacticDecision::Decision::Mixed;
    std::vector<Alphabet> chars(decision->chars.size());
    chars.resize(std::set_intersection(
                     decision->chars.begin(), decision->chars.end(),
...
bool AtomicTRE::makeSNF(const char singleC) {
  ...
  case op::concat: {
    if (std::all_of(list.begin(), list.end(),
                    [&](std::shared_ptr<AtomicTRE> tre) {
                      return tre->makeSNF(singleC);
                    })) {
      auto tmpTre = std::make_shared<SingletonTRE>(
          singleC,
          std::vector<std::shared_ptr<Interval>>{
              std::make_shared<Interval>(Bounds{0, true}, Bounds{0, true})});
...
bool DNFTRE::makeSNF(const char singleC) {
  auto tmpTre = std::make_shared<SingletonTRE>(
      singleC, std::vector<std::shared_ptr<Interval>>{});
  for (auto it = list.begin(); it != list.end();) {
    bool remainConjunctions = true;
    for (auto &expr : *it) {
      if (!(remainConjunctions =
                remainConjunctions && expr->makeSNF(singleC))) {
        break;
      }
    }
    if (remainConjunctions) {
  ...
  for (const auto &conjunctions : list) {
    std::vector<std::shared_ptr<Interval>> tmpIntervals = {
    ...
    tmpTre->intervals.insert(tmpTre->intervals.end(), tmpIntervals.begin(),
                             tmpIntervals.end());
  }
...
void AtomicTRE::toSignalTA(TimedAutomaton &out) const {
  ...
  case op::singleton: {
    ...
    for (const auto &interval : singleton->intervals) {
      ...
      guard.reserve(2);
      if (interval->lowerBound.first != 0 ||
          interval->lowerBound.second != true) {
        if (interval->lowerBound.second) {
          guard.emplace_back(TimedAutomaton::X(0) >=
                             interval->lowerBound.first);
        } else {
          guard.emplace_back(TimedAutomaton::X(0) > interval->lowerBound.first);
        }
        maxConstraint = std::max(maxConstraint,
                                 static_cast<int>(interval->lowerBound.first));
      }
      if (!std::isinf(interval->upperBound.first)) {
        if (interval->upperBound.second) {
          guard.emplace_back(TimedAutomaton::X(0) <=
                             interval->upperBound.first);
        } else {
          guard.emplace_back(TimedAutomaton::X(0) < interval->upperBound.first);
        }
        maxConstraint = std::max(maxConstraint,
                                 static_cast<int>(interval->upperBound.first));
      }
      out.states[0]->next[singleton->c].push_back(
          {out.states[1].get(), {}, guard});
    }
      ...
      for (auto &edges : s->next) {
        for (auto &edge : edges.second) {
          ...
          if (target && target->isMatch) {
            edges.second.reserve(edges.second.size() +
  ...
  case op::within: {
    ...
    for (auto initialState : out.initialStates) {
      for (const auto &initTransitionsPair : initialState->next) {
        ...
        dummyInitialState->next[c].reserve(dummyInitialState->next[c].size() +
                                           initTransitionsPair.second.size());
          ...
          if (target && target->isMatch) {
            ...
            if (within.second->upperBound.second) {
              transition.guard.emplace_back(
                  ...
                  within.second->upperBound.first);
            } else {
              transition.guard.emplace_back(TimedAutomaton::X(out.clockSize()) <
                                            within.second->upperBound.first);
            }
            // lower bound
            if (within.second->lowerBound.second) {
              transition.guard.emplace_back(
                  TimedAutomaton::X(out.clockSize()) >=
                  within.second->lowerBound.first);
            } else {
              transition.guard.emplace_back(TimedAutomaton::X(out.clockSize()) >
                                            within.second->lowerBound.first);
            }
